...
def extract_frames_through_directories(source_directory, prefix, destination_directory, frame_interval=10):
    ...
    for dirpath, dirnames, filenames in os.walk(source_directory):
        for filename in filenames:
            ...
            if source_file_path.endswith('.MOV'):
                ...
                extract_frames(source_file_path, destination_directory, prefix, index, frame_interval)
...
def videos_to_frames(video_path, prefix, frames_dir, index, target_size=(360, 640)):
    #extract_frames_through_directories(video_dir, prefix, frames_dir, 10)
...
def annotate_videos(video_dir, class_labels, out_dir):
    convert_json_filepath = os.path.join(video_dir, "convert.json")    
...
#videos_to_frames(f"{HOME}/videos", f"{HOME}/tmp")
#target_class_id = class_labels.index(class_label_to)
#annotate_image(f"{HOME}/images/images_IMG_7010.jpg", class_label_from, target_class_id)

#generate_yaml(class_labels, f"{HOME}/dataset/train", f"{HOME}/dataset/valid", f"{HOME}/dataset/data.yaml")


annotate_videos(f"{HOME}/../videos/fruits/lime", class_labels, f"{HOME}/dataset/train")
