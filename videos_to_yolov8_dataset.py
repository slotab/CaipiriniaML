    ...
    for dirpath, dirnames, filenames in os.walk(source_directory):
        for filename in filenames:
            ...
            if source_file_path.endswith('.MOV'):
                extract_frames(source_file_path, destination_directory, dirpath.split('/')[-1], index, frame_interval)
...
def move_files(image_list, src_img_dir, src_lbl_dir, dest_img_dir, dest_lbl_dir):
    ...
    for image_file in image_list:
        ...
        lbl_src_path = os.path.join(src_lbl_dir, image_file.replace('.jpg', '.txt'))  # Remplace jpg par txt pour trouver le fichier d'annotation
...
def split_dataset(image_dir, annotation_dir, output_directory, ratios=(0.7, 0.2, 0.1)):
    ...
    move_files(train_files, image_dir, annotation_dir, os.path.join(output_directory, 'train', 'images'), os.path.join(output_directory, 'train', 'labels'))
    move_files(valid_files, image_dir, annotation_dir, os.path.join(output_directory, 'valid', 'images'), os.path.join(output_directory, 'valid', 'labels'))
    move_files(test_files, image_dir, annotation_dir, os.path.join(output_directory, 'test', 'images'), os.path.join(output_directory, 'test', 'labels'))

    print(f"Répartition terminée : {len(train_files)} train, {len(valid_files)} valid, {len(test_files)} test.")



def generate_yaml(output_directory, dataset_name, classes, yaml_output_path):
    """
...
def get_class_names(root_directory):
    ...
    classes_names = [name.split('_')[0] for name in os.listdir(root_directory) if os.path.isdir(os.path.join(root_directory, name))]
    # classes_names = []
...
frames_dir = '../tmp'
datasets_dir = '../out/yolo/datasets'
